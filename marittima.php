<!-- Instruktion til webbrowser om at vi kører HTML5 -->
<!DOCTYPE html>

<!-- html starter og slutter hele dokumentet / lang=da: Fortæller siden er på dansk -->
<html class="h-100" lang="da">

<!-- I <head> har man opsætning - det ser brugeren ikke, men det fortæller noget om siden -->
<head>
    <!-- Sætter tegnsætning til utf-8 som bl.a. tillader danske bogstaver -->
    <meta charset="utf-8">

    <!-- Titel som ses oppe i browserens tab mv. -->
    <title>Lasellah | Marittima projekt</title>
    <meta name="description" content=" Vinoteque Marittima er en event båd, med et stort aktivitetsprogram.
    Vinsmagning, poker aftener, casino nights og fester er blandt årets kommende events på båden.">
    <!-- Metatags der fortæller at søgemaskiner er velkomne, hvem der udgiver siden og copyright information -->
    <meta name="robots" content="All">
    <meta name="author" content="Udgiver">
    <meta name="copyright" content="Information om copyright">

    <!-- Sikrer man kan benytte CSS ved at tilkoble en CSS fil -->
    <link href="css/bootstrap.css" rel="stylesheet" type="text/css">
    <link href="css/styles.css" rel="stylesheet" type="text/css">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Electrolize&display=swap" rel="stylesheet">
    <script src="https://kit.fontawesome.com/b7f7f27b49.js" crossorigin="anonymous"></script>
    <!-- Sikrer den vises korrekt på mobil, tablet mv. ved at tage ift. skærmstørrelse - bliver brugt til responsive websider -->
    <meta name="viewport" content="width=device-width, initial-scale=1">
</head>

<!-- i <body> har man alt indhold på siden som brugeren kan se -->
<body class="h-100 bg-primary">
<?php include "includes/nav.php"; ?>

<div class="justify-content-center d-flex">
    <img class="w-25 pt-5" src="images/logo-Forside-Hero.png" alt="logo">
</div>
<div>
    <h1 class="text-light p-4 text-center">
       Marittima
    </h1>
</div>
<!--link til hjemmeside-->
<div class="text-center">
    <a class="text-decoration-none" href="https://github.com/loay0013/marittima" target="_blank">
        <h2 class="bg-link">>/Link til Github></h2></a>
</div>
<!--text-->
<h2 class="text-light text-center pt-4">
    >/2.semester projekt Multimediedesigner>
</h2>
<div class="container">
<p class="text-center text-light  mt-4 p-4">
    " Vinoteque Marittima" er en event båd, med et stort aktivitetsprogram.
    Vinsmagning, poker aftener, casino nights og fester er blandt årets kommende events på båden.
    Projektet gik ud på, at skabe en hjemmeside og SoMe kampagne til virksomheden.
    Min rolle i projektet var, at kode hjemmesiden og at filme reklame video til SoMe.</p>
</div>

<!--link næste projekt-->
<div class="text-center pb-5">
    <a class="text-decoration-none næste"  href="DinIndreJuvel.php"><h2>>/Næste projekt></h2></a>
</div>
<?php include "includes/footer.php"; ?>


<script src="node_modules/bootstrap/dist/js/bootstrap.bundle.min.js"></script>
</body>
</html>
